...
(function () {
  ...
  // Catch anything that slips past a stage's own try/catch (there isn't
  // one per-stage by design — keeping the animation code simple — so this
  // is the backstop) and report it visibly instead of leaving the machine
  // stuck silently mid-stage.
  window.addEventListener("error", (e) => {
    if (window.__resumeMachineBootFailed) return;
    window.__resumeMachineBootFailed = true;
  ...
  function reportRuntimeError(err) {
    try {
      ...
      bar.style.cssText =
        "position:fixed;left:0;right:0;top:0;z-index:999;background:#c1341f;color:#fff;" +
        "font:12px/1.5 monospace;padding:10px 14px;text-align:center;";
      bar.textContent =
        "The Resume Machine hit an error (" +
        (err && err.message ? err.message : String(err)) +
        "). SKIP TO RESUME still works.";
      document.body.appendChild(bar);
      const skip = document.getElementById("skipBtn");
      if (skip) skip.href = CONFIG.resumeURL;
    } catch (_) { /* nothing more we can do */ }
    if (window.console && console.error) console.error("Resume Machine error:", err);
  }
  ...
  /* ---------------- boot + visible failure reporting ----------------
     If anything about the page/script pairing is wrong (a stale cache,
     a case-mismatched filename on a case-sensitive host, a required
     element missing), fail loudly on-page instead of leaving a
     good-looking machine with dead buttons and no clue why. */
  function boot() {
    try {
      initializeMachine();
    } catch (err) {
  ...
  if (document.readyState === "loading") {
    ...
    // Script ran after the DOM was already parsed (e.g. injected late,
    // cached oddly, or DOMContentLoaded already fired) — boot immediately
